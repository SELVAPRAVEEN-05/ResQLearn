...
export async function POST(
  ...
  try {
    ...
    const {
      ...
      course_id: courseId,
      string_lesson_id: stringLessonId,
    } = matRes.rows[0];
    ...
    if (lessonCompleted) {
      ...
      courseProgressPercent =
        totalCourseLessons > 0
          ? Math.min(
              100,
              Math.round((completedLessonCount / totalCourseLessons) * 100),
            )
          : 100;
      ...
      const completedIds = allLessonsRes.rows
        .filter((l) =>
          completedLessonsRes.rows.some((cl) => cl.lesson_id === l.id),
        )
        .map((l) => String(l.id));
    ...
    return NextResponse.json(
      { error: "Failed to update material completion" },
      { status: 500 },
    );
  }
}
