...
export async function GET() {
  ...
  try {
    const studentsRes = await query(
      "SELECT COUNT(*)::int as count FROM resq_users WHERE role = 'student'",
    );
    const totalUsersRes = await query(
      "SELECT COUNT(*)::int as count FROM resq_users",
    );
    const activeUsersRes = await query(
      "SELECT COUNT(*)::int as count FROM resq_users WHERE status = 'Active'",
    );

    const coursesRes = await query(
      "SELECT COUNT(*)::int as total, COUNT(CASE WHEN is_published = TRUE THEN 1 END)::int as published FROM resq_courses",
    );
    const lessonsRes = await query(
      "SELECT COUNT(*)::int as count FROM resq_lessons",
    );
    const materialsRes = await query(
      "SELECT COUNT(*)::int as count FROM resq_materials",
    );

    const completedCoursesRes = await query(
      "SELECT COUNT(*)::int as count FROM resq_user_course_progress WHERE completed = TRUE",
    );
    const avgCourseProgressRes = await query(
      "SELECT AVG(progress_percent)::float as avg_progress FROM resq_user_course_progress",
    );
    ...
    const averageQuizScore = qaRow.avg_score
      ? Math.round(Number(qaRow.avg_score) * 100)
      : 0;
    ...
    return NextResponse.json(
      { error: "Failed to calculate admin stats" },
      { status: 500 },
    );
  }
}
