...
export async function GET() {
  ...
  const startedCourses = publishedCourses.filter(
  ...
  const totalCourseProgressSum = publishedCourses.reduce(
    (acc, c) => acc + Number(c.progress || 0),
    0,
  );
  const overallLearningProgress =
    availableCoursesCount > 0
      ? Math.round(totalCourseProgressSum / availableCoursesCount)
      : 0;

  const attempts = attemptsRes.rows;
  const totalAttempts = attempts.length;
  const totalScoreRatio = attempts.reduce(
    (acc, a) => acc + a.score / (a.total || 1),
    0,
  );
  const avgQuizScore =
    totalAttempts > 0 ? Math.round((totalScoreRatio / totalAttempts) * 100) : 0;
  const bestQuizScore =
    totalAttempts > 0
      ? Math.round(
          Math.max(...attempts.map((a) => a.score / (a.total || 1))) * 100,
        )
      : 0;

  const inProgressCourse =
    startedCourses[0] ||
    (completedCoursesCount < availableCoursesCount
      ? publishedCourses.find((c) => Number(c.progress) === 0)
      : publishedCourses[0]) ||
    null;
