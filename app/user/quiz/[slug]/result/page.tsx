...
export default function QuizResultPage(props: {
  ...
  const scorePercent =
    attempt.percentage ||
    Math.round((attempt.score / (attempt.total || 1)) * 100);
  ...
  const stats = [
    ...
    {
      label: "Incorrect",
      value: (attempt.total - attempt.score).toString(),
      icon: XCircle,
      iconColor: "#DC2626",
    },
    {
      label: "Status",
      value: isPassed ? "PASSED" : "RETAKE",
      icon: Timer,
      iconColor: isPassed ? "#10B981" : "#F59E0B",
    },
  ];
  ...
  return (
    <section className="space-y-5 animate-[fadeIn_0.5s_ease-out] pb-10">
      ...
      <div className="grid grid-cols-2 gap-3">
        {stats.map((s) => {
          ...
          return (
            <div
              key={s.label}
              className="rounded-2xl border border-[#E5E7EB] bg-white p-4 text-center shadow-sm"
            >
              <Icon
                className="mx-auto"
                size={18}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-[#111827]">
                {attempt.category || quiz?.category} Safety Certification
                Earned!
              </p>
              <p className="mt-1 text-xs leading-5 text-[#6B7280]">
                You have met the verified standard for disaster preparedness.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="rounded-3xl border border-[#E5E7EB] bg-white p-5 shadow-sm space-y-4 animate-[fadeIn_0.3s_ease-out]">
          ...
          <div className="space-y-4">
            {quiz.questions.map((q, idx) => {
              const reviewItem = attempt.answers?.find(
                (a) => a.questionId === q.id,
              );
              ...
              return (
                <div
                  key={q.id}
                  className="rounded-2xl border border-[#E5E7EB] p-4 bg-[#F9FAFB] space-y-2"
                >
                  <div className="flex items-center justify-between">
                    ...
                    {isCorrect !== undefined && (
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isCorrect ? "bg-[#ECFDF5] text-[#059669]" : "bg-[#FEE2E2] text-[#DC2626]"}`}
                      >
                  ...
                  <div className="space-y-1.5 pt-1">
                    {q.options.map((opt, oIdx) => {
                      const isOptionCorrect =
                        oIdx === q.options.indexOf(opt) && reviewItem
                          ? oIdx === reviewItem.correctAnswerIndex
                          : false;
                  ...
                  {q.explanation && (
                    <p className="text-xs text-[#6B7280] bg-white p-2.5 rounded-xl border border-[#E5E7EB] mt-2 leading-relaxed">
                      <strong className="text-[#111827]">Explanation: </strong>{" "}
                      {q.explanation}
                    </p>
                  )}
                </div>
      ...
      <div className="space-y-3">
        ...
        >
          <Eye size={16} />{" "}
          {showReview ? "Hide Answers Review" : "Review Questions & Answers"}
        </button>
          ...
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl space-y-4 text-center border-4 border-[#10B981]/20">
            ...
            <div>
              ...
              <h2 className="text-xl font-extrabold text-[#111827] mt-1">
                {attempt.category || quiz?.category} Emergency Preparedness
              </h2>
              <p className="text-xs text-[#6B7280] mt-1">
                Issued by SafeGraph AI Emergency Response Academy
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-[#E5E7EB] bg-[#F9FAFB] p-4 text-xs text-[#374151] space-y-1">
              ...
              <p className="text-xs text-[#6B7280]">
                has achieved a passing score of <strong>{scorePercent}%</strong>{" "}
                on the official {attempt.quizTitle || quiz?.title}.
              </p>
            </div>
