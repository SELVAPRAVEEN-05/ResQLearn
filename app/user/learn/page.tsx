...
const getCategoryIcon = (disasterType: string, iconName?: string) => {
  ...
  if (t.includes("cyclone") || iconName === "Wind" || iconName === "Zap")
    return Wind;
...
export default function LearnPage() {
  ...
  const filteredCourses = courses.filter((c) => {
    ...
    const matchesCategory =
      ...
      (c.disasterType &&
        c.disasterType.toLowerCase() === selectedCategory.toLowerCase()) ||
      ((c as any).category &&
        (c as any).category.toLowerCase() === selectedCategory.toLowerCase());
    ...
    <section className="space-y-6 animate-[fadeIn_0.5s_ease-out] pb-10">
      <div>
        ...
        <p className="mt-1 text-sm leading-6 text-[#6B7280]">
          Equip yourself with verified, life-saving knowledge and interactive
          emergency protocols.
        </p>
      </div>
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-dashed border-[#D1D5DB] p-10 text-center bg-white">
            <BookOpen className="mx-auto text-[#9CA3AF] mb-3" size={36} />
            <h3 className="text-base font-bold text-[#111827]">
              No courses available yet.
            </h3>
            <p className="text-xs text-[#6B7280] mt-1 max-w-sm mx-auto">
              There are no published courses matching your search or category
              filter. Check back soon for new modules.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {filteredCourses.map((course) => {
              const disaster =
                course.disasterType || (course as any).category || "Disaster";
                ...
                >
                  ...
                  <div className="mt-3 flex items-center gap-4 text-xs text-[#6B7280]">
                    <span className="flex items-center gap-1 font-semibold text-[#374151]">
                      <BookOpen className="text-[#10B981]" size={13} />{" "}
                      {lessonCount} Lessons
                    </span>
                    <span className="flex items-center gap-1 font-semibold text-[#374151]">
                      <Clock className="text-[#10B981]" size={13} />{" "}
