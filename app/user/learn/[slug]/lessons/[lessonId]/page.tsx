  ...
  switch (type?.toUpperCase()) {
    ...
    default:
      return {
        icon: File,
        label: "RESOURCE",
        color: "bg-[#F3F4F6] text-[#4B5563] border-[#E5E7EB]",
      };
  }
};
...
export default function LessonPage(props: {
  ...
  const [materialCompletionMap, setMaterialCompletionMap] = useState<
  ...
  const [activeVideoId, setActiveVideoId] = useState<number | string | null>(
    null,
  );
    ...
    const fetchLessonData = async () => {
      ...
      if (res.course && res.course.lessons) {
        ...
        const idx = res.course.lessons.findIndex(
          (l) =>
            String(l.id) === params.lessonId || l.lessonId === params.lessonId,
        );
  ...
  const videoMaterials =
    currentLesson.materials?.filter((m) => m.type === "VIDEO") || [];
  const selectedVideo =
    videoMaterials.find((v) => v.id === activeVideoId) ||
    videoMaterials[0] ||
    null;
  ...
  return (
    <section className="space-y-6 animate-[fadeIn_0.5s_ease-out] pb-16">
      ...
      {selectedVideo && (
        <div
          className="overflow-hidden rounded-3xl border border-[#E5E7EB] bg-black shadow-lg"
        ...
        >
          {selectedEmbedUrl ? (
            ...
            <div className="p-8 text-center bg-[#111827] text-white space-y-3">
              <FileVideo className="mx-auto text-[#3B82F6]" size={42} />
              <h3 className="text-base font-bold">{selectedVideo.title}</h3>
              <p className="text-xs text-white/70 max-w-md mx-auto">
                {selectedVideo.description ||
                  "Click below to watch this educational video in full quality."}
              </p>
              <a
          ...
          <div className="p-3.5 bg-[#111827] text-white flex flex-wrap items-center justify-between gap-2 text-xs">
            <div className="flex items-center gap-2 overflow-hidden">
              <span className="rounded bg-[#3B82F6] px-2 py-0.5 text-[10px] font-bold uppercase">
                VIDEO
              </span>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-[#6B7280] font-semibold">
              {currentLesson.materials.length} Resources
            </span>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {currentLesson.materials.map((mat) => {
              ...
              const isVid =
                mat.type === "VIDEO" || !!getYouTubeEmbedUrl(mat.url);
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    >
                      {isTogglingThis ? (
                        ...
                        <>
                          <CheckCircle2 className="text-[#059669]" size={13} />
                          <span>Marked as Completed</span>
                        </>
                      ) : (
                        <>
                          <CheckCircle2 className="text-[#9CA3AF]" size={13} />
      ...
      <div className="space-y-3 pt-4 border-t border-[#E5E7EB]">
        {!isCompleted ? (
          ...
          >
            {completing ? (
              <>
                <RotateCw className="animate-spin" size={16} /> Saving
                Progress...
              </>
            ) : (
              <>
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/65 p-2 sm:p-4 backdrop-blur-xs animate-[fadeIn_0.2s_ease-out]">
          <div className="flex h-full max-h-[95vh] w-full max-w-4xl flex-col rounded-3xl bg-white shadow-2xl overflow-hidden border border-[#E5E7EB]">
            ...
            <div className="flex items-center justify-between border-b border-[#374151] bg-[#111827] px-5 py-3 text-white">
              <div className="flex items-center gap-2 overflow-hidden">
                <FileText className="text-[#EF4444] shrink-0" size={18} />
                ...
                <button
                  ...
                  onClick={() =>
                    setActiveViewerModal((prev) => ({
                      ...prev,
                      viewerEngine:
                        prev.viewerEngine === "native" ? "google" : "native",
                    }))
                  }
                >
                  <RefreshCw size={11} />
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex h-full flex-col items-center justify-center p-8 text-center bg-white rounded-2xl space-y-3">
                      <FileText className="text-[#EF4444]" size={48} />
                      <h4 className="text-base font-bold text-[#111827]">
                        {activeViewerModal.material.title}
                      </h4>
                      <p className="text-xs text-[#6B7280] max-w-sm">
                        This PDF is ready to view. Click below to open the
                        complete document in your browser or download it.
                      </p>
